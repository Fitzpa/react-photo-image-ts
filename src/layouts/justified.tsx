...
const getCommonHeight = (row:any, containerWidth:number, margin:number) => {
  const rowWidth = containerWidth - row.length * (margin * 2);
  const totalAspectRatio = row.reduce((acc:any, photo:IPhoto) => acc + ratio(photo), 0);
  return rowWidth / totalAspectRatio;
};

// calculate the cost of breaking at this node (edge weight)
const cost = (photos:IPhoto[], i:number, j:number, width:number, targetHeight:number, margin:number) => {
...
const makeGetNeighbors = (targetHeight:number, containerWidth:number, photos:IPhoto[], limitNodeSearch:number, margin:number) => {
  const results:any = {};
  const start = 0
  results[start] = 0;
